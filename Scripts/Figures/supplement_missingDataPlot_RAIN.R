...
#################################
setwd("~/Desktop/TimeCycle-data/Results/MissingData/RAIN/")
...
##############################################################
# LOAD META DATA INTO R SAME FOR RAIN
##############################################################

metaInfo <- regmatches(names(AUC_RAIN), gregexpr("[[:digit:]]+\\.*[[:digit:]]*", names(AUC_RAIN)))
metaInfo <- do.call(rbind, metaInfo)
noiseLevels <- as.numeric(metaInfo[, 3])
missingness <- as.numeric(metaInfo[, 5])

plotData <- data.frame(AUC_RAIN, noiseLevels, missingness)
## --------------------------------------- Plot Results --------------------------------

pdf(file = "~/Desktop/TimeCycle-data/Results/Figures/supp_missingness_RAIN.pdf", width = 10, height = 8)
...
for (i in keep) {
  ...
  wave <- gsub(pattern = "_.*", replacement = "", rownames(RAINresults))
  RAINresults$wave <- wave
  RAINresults$wave <- fct_relevel(wave, ordering)
  RAINresults$pVal_log <- -log(RAINresults$pVal)

  p[[count]] <- ggplot(RAINresults, aes(x = pVal_log, fill = wave, color = wave)) +
    geom_histogram() +
