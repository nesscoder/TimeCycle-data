...
#################################
# LOAD THE DATA INTO R JTK_CYCLE
#################################
setwd("~/Desktop/TimeCycle-data/Results/MissingData/JTK/")
...
###############################
# get the adjusted pvalues and make them into a single data.frame
...
##############################################################
# LOAD META DATA INTO R SAME FOR JTK_CYCLE
##############################################################

metaInfo <- regmatches(names(AUC_JTK), gregexpr("[[:digit:]]+\\.*[[:digit:]]*", names(AUC_JTK)))
metaInfo <- do.call(rbind, metaInfo)
noiseLevels <- as.numeric(metaInfo[, 3])
missingness <- as.numeric(metaInfo[, 5])

plotData <- data.frame(AUC_JTK, noiseLevels, missingness)
## --------------------------------------- Plot Results --------------------------------

pdf(file = "~/Desktop/TimeCycle-data/Results/Figures/supp_missingness_JTK.pdf", width = 10, height = 8)
...
for (i in keep) {
  ...
  wave <- gsub(pattern = "_.*", replacement = "", rownames(JTKresults))
  JTKresults$wave <- wave
  JTKresults$wave <- fct_relevel(wave, ordering)
  JTKresults$pVal_log <- -log(JTKresults$ADJ.P)

  p[[count]] <- ggplot(JTKresults, aes(x = pVal_log, fill = wave, color = wave)) +
    geom_histogram() +
