...
selData <- function(data, ZT_start) {
  sel <- seq(from = ZT_start, by = 1, length.out = 19)
  return(data[, sel])
}
...
process_JTK <- function(data) {
  annot <- rownames(data)
  jtkdist(19, 1) # 21 total time points, 1 replicates per time point
...
##########################################
# Sliding Window 36 Hour Time Comparison
##########################################
# ZT times to cycle through starting at 1-12
ZT_startTimes <- 1:6

# Run Analysis on 36 hour windows using Timecycle and Save the Adjusted p-values
...
# Run Analysis on 36 hour windows using JTK_CYCLE and Save the Adjusted p-values
...
# Run Analysis on 36 hour windows using RAIN and Save the Adjusted p-values
...
##########################################
# 36 Hour Same ZT Time Comparison
##########################################
...
Hughes <- data_hughes[, paste0("ZT", c(seq(18,46,2),seq(0,6,2)))] #hughes dataset need to wrap around
...
save.image("~/Desktop/TimeCycleV3/Results/SlidingWindows/36HourComparison_Zhang_Ceiling.Rdata")
