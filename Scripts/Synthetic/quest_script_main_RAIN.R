...
st <- system.time({
results <- rain(t(data), deltat = as.numeric(args[4]), period = as.numeric(args[5]), nr.series = as.numeric(args[6]), adjp.method = "rawp",peak.border = c(as.numeric(args[7]),as.numeric(args[8])), verbose = F)
})
